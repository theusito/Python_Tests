...
class Gui():
    ...
    # Definição das variáveis que recebem os dados pelo user
    txtNome = StringVar()
    ...
    btnBuscar.grid(row=5, column=0, rowspan=2)
    btnInserir.grid(row=6, column=0, rowspan=2)
    btnUpdate.grid(row=7, column=0, rowspan=2)
    btnDel.grid(row=8, column=0, rowspan=2)
    btnClose.grid(row=9, column=0, rowspan=2)
    ...
    #Adicionar SWAG(estilo) à interface
    for child in window.winfo_children():
        widget_class = child.__class__.__name__
        if widget_class == "Button":
            child.grid_configure(stricky='WE', padx=x_pad, pady=y_pad)
        elif widget_class == "Listbox":
            child.grid_configure(padx=0, pady=0, sticky='NS')
        elif widget_class == "Scrollbox":
            child.grid_configure(padx=0, pady=0, sticky='NS')
        else:
